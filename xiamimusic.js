...
var $=$ || unsafeWindow.$,iframe=$('<div style="display:none;">').html('<iframe id="xiamihelper"></iframe>').appendTo(document.body).find('#xiamihelper')[0],
...
msg=[
    ...
    '您是虾米VIP会员,您可以直接下载高品质音乐,您也可以 <a style="text-decoration:underline;" href="javascript:void(0);" onclick=\'window.open("'+shareurl+'");\' >点此</a> 分享您的Cookie----人人喂我,我喂人人<div><a style="text-decoration:underline;color:#999999;" href="javascript:void(0);" onclick=\'window.open("'+sharelisturl+'");\'>贡献者名单</a></div>',//1
...
function getSongList(){
    ...
    for(var i=1;i<o.length;i++){
        var song=getSongInfo($(o[i]).find('a')[0]);
...
function showUerInfo(text){
    var html='<div align="center" style="color:#008000;"><b>'+text+'</b></div>',node=this.node;
    if(node){return node.html(html);}
    var o=$('#song_count').parent().parent()[0];
    this.node=$('<span>').html(html).appendTo(o);
...
function showSongsInfo(o,opt){
    ...
    if(node){
        node.html('<a href="javascript:;">'+msg[5]+'</a>').find('a').css({'float':'right','position':'relative','margin-right':'30px','text-decoration':'underline'}).click(function(){iframe.src=url;});
        return opt && opt.msg && showUerInfo(opt.msg);
    }
    o.node=$('<span>').html('<span style="float:right;position:relative;margin-right:10px;color:#A1CBE4;">'+msg[4]+'...</span>').appendTo(o.parentNode);
}
